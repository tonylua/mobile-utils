...
/***/ function(module, exports) {
	...
	function form_primary_valid(form) {
	    ...
	    var submitTriggerEvent = arguments.length <= 2 || arguments[2] === undefined ? 'click' : arguments[2];
	    var submitTriggerTarget = arguments.length <= 3 || arguments[3] === undefined ? '.submit' : arguments[3];
	    var mismatchNoticeName = arguments.length <= 4 || arguments[4] === undefined ? 'mismatch' : arguments[4];
	        ...
	        while (fields.length) {
	            ...
	            if (!fake.checkValidity()) {
	                if (ipt.hasAttribute('required') && ipt.validity.valueMissing) //空缺
	                    ipt.setCustomValidity('');else if (ipt.dataset.hasOwnProperty(mismatchNoticeName)) //不符合pattern
